...
function FunctionsQuickstart() {
  ...
  this.firNumber = document.getElementById('firstnumber');
  this.secNumber = document.getElementById('secondnumber');
  this.minusNumbersButton = document.getElementById('send-minus-button');
  ...
  this.minusNumbersButton.addEventListener('click',this.minusNumbers.bind(this));
...
// Minus
FunctionsQuickstart.prototype.minusNumbers = function() {
  var firstNumber = parseFloat(this.firNumber.value);
  var secondNumber = parseFloat(this.secNumber.value);
  var minusNumbersButton = this.minusNumbersButton;
  minusNumbersButton.disabled = true;
  // [START callAddFunction]
  var sendNotification = firebase.functions().httpsCallable('minusNumbers');
  sendNotification({firstNumber: firstNumber, secondNumber: secondNumber}).then(function(result) {
    console.log('Cloud Function called successfully.', result);
    // Read results of the Cloud Function.
    var firstNumber = result.data.firstNumber;
    ...
    window.alert('Here is the result of the formula: ' + firstNumber + ' '
        + operator + ' ' + secondNumber + ' = ' + operationResult);
    minusNumbersButton.disabled = false;
    // [END_EXCLUDE]
  }).catch(function(error) {
    // Getting the Error details.
    var code = error.code;
